	...
	return {
			 ...
			 var delimeter=">>";
				...
				getDepartementsPromise.then(
						function(data){
				        	...
				        	console.log('size of departements array : '+objearraySize);
						...
						function(error) {
							//scope.merchDataLoading=false;
							console.log("Department Data not found from WS Call");
						}
				);
				// Classes code STARTs
				...
				scope.getClassesforSelectedDepartment=function(){
					...
					getClassesPromise.then(
							...
							function(error) {
								scope.merchDataLoading=false;
								console.log("Class Data not found from WS Call");
							});
				}
				}
				...
				scope.getSubClasses=function(){
					...
					getSubClassesPromise.then(
							...
							function(error) {
								scope.merchDataLoading=false;
								console.log("SubClass Data not found from WS Call");
							});
				}
				}
				// getSubClassess Method - START
		...
		if(scope.selectedDept==null) 
				...
				if(scope.tableData.length===0) {
					...
					//jsondata.name = scope.selectedDept.name+"~"+className+"~"+subClassName;
					//jsondata.id = scope.selectedDept.id+"~"+classNumber+"~"+subClassNumber;
					...
					//console.log('____jsondata.nam',jsondata.name);
					//console.log('____jsondata.id',jsondata.id);
					//console.log('____jsondata.catalog',jsondata.catalog);
					//console.log('____jsondata',jsondata);
			...
			if(dataObject && dataObject!=null) {
				...
				//var	nameArray=dataObject.name.split("~");
				//var	idArray=dataObject.id.split("~");
